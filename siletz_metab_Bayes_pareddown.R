...
sub1 = "001_data\\metabolism\\"

# Uses data that accounts for depth of water over the instrument

x <- readRDS(paste0(dir, sub1, "data.DO.metab.NAreplce_2.RData"))
...
for (i in 1 : length(STAID))
{
    ...
    if (df.dim[1] == 0)
    {
        ...
        y <- metab(specs = specs(mm_name(type = 'bayes')), data = xsub)
        
        metab.output.all[[1]][[i]] <- y
        
    }
    
    print(paste0("Station ", STA[i], ", Cold-water at ", Sys.time()," -- ",
    ...
    print(paste0("Station ", STA[i], ", Spawning at ", Sys.time()," -- ",
                 round(100 * perc.comp, 0), "% COMPLETE"))
}
...
saveRDS(metab.output.all, paste0(dir, sub1, "Metab_Output_Bayes.RData"))
