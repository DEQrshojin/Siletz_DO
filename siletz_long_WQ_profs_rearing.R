...
data.tmp <- data.tmp[which(data.tmp$STAID != 38941), ] # Remove Strome Park 38941
...
data.bxplt$STAID <- factor(data.bxplt$STAID,
                           levels(factor(data.bxplt$STAID))[c(12, 3, 6, 1,  5,  8,
                                                               9, 2, 4, 7, 11, 10)])
...
# PLOT DO CONCENTRATION (mg/L)
    ...
    for (i in 1 : length(seas))
    {
        box.Siletz[[i]] <- ggplot() +
            ...
            annotate("text", 53, 8.15, label = "Cold-water 30d mean min, 8.0 mg/L",
                     hjust = 0, size = 3.0) +
            annotate("text", 53, 6.65, label = "Cold-water 7d min mean, 6.5 mg/L",
                     hjust = 0, size = 3.0) +
            annotate("text", 53, 6.15, label = "Cold-water absolute min, 6.0 mg/L",
                     hjust = 0, size = 3.0)
...
# DO CONCENTRATION (% SAT)---------------------------
    ...
    for (i in 1 : length(seas))
    {
        box.Siletz[[i]] <- ggplot() +
            ...
            annotate("text", 53, 89, color = "black",
                     label = "Cold-water Minimum, 90 percent", hjust = 0, vjust = 1, size = 3) +
