...
data.tmp <- data.tmp[complete.cases(data.tmp[, 5]), ] # Remove NA
...
# Remove NA values
# temp.7d.mx.gr <- temp.7d.mx.gr[complete.cases(temp.7d.mx.gr[, 3]), ]
# Rename the values in the Station column
sites.old <- unique(temp.7d.mx.gr$Station)
data.site <- read.csv(paste0(data.dir, "sites_order_z_TEMP.csv"))
...
sta.order <- c(12, 13, 3, 1, 6, 5, 8, 9, 2, 4, 7, 11, 10)
...
ggsave(filename = "fig20_lswcd_temp_7dadm_all.jpg", plot = temp.7d.mx.plot, path = save.dir, width = 8, height = 6, units = "in", dpi = 300)
