...
# DO CONCENTRATION (mg/L)---------------------------
    ...
    for (i in 1 : length(seas))
    {
        box.Siletz[[i]] <- ggplot() + geom_boxplot(data = subset(data.bxplt, (SEASON == seas[i] & River == "Siletz River")),
            ...
            annotate("text", 53, 10.65, label = "Spawning 7d mean min,\n11.0 mg/L", hjust = 0, size = 3.0)
        ...
        ggsave(paste0("fig", j, "_do_conc_bxplt_", seas[i], ".png"), plot = x, path = paste0(dir, dir.sub3), scale = 1,
...
# DO CONCENTRATION (% SAT)---------------------------
    ...
    for (i in 1 : length(seas))
    {
        box.Siletz[[i]] <- ggplot() + geom_boxplot(data = subset(data.bxplt, (SEASON == seas[i] & River == "Siletz River")),
            ...
            geom_segment(aes(x = 46.9, y = 60, xend = 46.9, yend = 76.25), color = "gray48", size = 1,
                         arrow = arrow(length = unit(0.05, "inches"))) +
            annotate("text", 53, 91, color = "black",
                     label = "Spawning Minimum,\n95 percent", hjust = 0, size = 3) +
        ...
        box.Rock[[i]] <- ggplot() + geom_boxplot(data = subset(data.bxplt, (SEASON == seas[i] & River == "Rock Creek")),
            ...
            scale_x_continuous(limits = c(-1, 3), breaks = c(0, 1, 2, 3)) +
            scale_y_continuous(limits = c(50, 175), breaks = c(50, 75, 100, 125, 150)) +
            xlab("River Mile") + ylab("Dissolved Oxygen\n(% Saturation)") + ggtitle("Rock Creek") +
            annotate("text", x = stations$RKm, y = 143.75, label = paste0(stations$STAID, " - ", stations$Site_ID), angle = 90, size = 3,
                     hjust = 0) +
            annotate("text", x = 46.9, y = 56.25, label = "Rock Creek\nConfluence", size = 3) +
            geom_segment(aes(x = 46.9, y = 60, xend = 46.9, yend = 76.25), color = "gray48", size = 1,
...
# TEMPERATURE (degC)-----------------------------------
    ...
    for (i in 1 : length(seas))
    {
        box.Siletz[[i]] <- ggplot() + geom_boxplot(data = subset(data.bxplt, (SEASON == seas[i] & River == "Siletz River")),
            ...
            geom_segment(aes(x = 46.9, y = 3, xend = 46.9, yend = 7.875), color = "gray48", size = 1,
                         arrow = arrow(length = unit(0.05, "inches"))) +
            annotate("text", 53, 11, color = "black",
                     label = "Spawning 7DAD\nMaximum, 13°C", hjust = 0, size = 3.0) +
