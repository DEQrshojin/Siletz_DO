# Modified by Ryan Shojinaga 06/22/2018
# Original by Dan Sobota
...
# THIS SECTION DEALS WITH THE DATA COLLECTED IN THE SUMMER PERIOD
#________________________________________________________________
...
sub.dir3 <- "005_reporting\\figures\\"
...
data.R$DATE_TIME <- as.POSIXct(paste0(data.R$Date..MM.DD.YYYY., data.R$Time..HH.MM.SS.), format="%m/%d/%Y %H:%M:%S") - hours(7)

# Rename Columns and reorder
data.R <- data.R[ -c(1 : 3, 5 : 7)]
names(data.R) <- c("Station", "TempC", "SpCon", "DOmgL", "DOSat", "pH", "Turb", "TSSmgL", "Depm","DATE_TIME")
...
doconc.R.plots <-  ggplot(data = data.R, aes(x = DATE_TIME, y = DOmgL, group = Station, color = Station)) + 
...
temp.R.plots <-  ggplot(data = data.R, aes(x = DATE_TIME, y = TempC, group = Station, color = Station)) +
             ...
             annotate("text", lims.1 + 86400 * 1, 15, color = "black",
                      label = "Rearing 7DAD Maximum, 16°C", hjust = 0, size = 2.4) +
...
turb.R.plots <-  ggplot(data = data.R, aes(x = DATE_TIME, y = Turb, group = Station, color = Station)) +
             ...
             theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor=element_blank(),
                                ...
                                legend.text = element_text(size  = 8)) # + guides(col = guide_legend(ncol = 3)) 
...
doconc.S.plots <-  ggplot(data = data.S, aes(x = DATE_TIME, y = DOmgL, group = Station, color = Station)) + 
    ...
    annotate("text", lims.1 + 86400 * 1, 11.6, label = "Spawning 7d mean min, 11.0 mg/L", hjust = 0, size = 2.4)
# DO SATURATION PLOT   --------------------------------------------------------------------------------------
dosat.S.plots <- ggplot(data = data.S, aes(x = DATE_TIME, y = DOSat, group = Station, color = Station)) +
    ...
    annotate("text", lims.1 + 86400 * 1, 91, color = "black",
             label = "Spawning Minimum, 95 percent", hjust = 0, size = 2.4) +
